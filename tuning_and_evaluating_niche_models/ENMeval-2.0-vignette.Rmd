...
```{r}
library(sf)
# Note from TJS/EES. Here we will use the package 'blockCV' to partition our data. The latest version of blockCV has been updated to work with 
# terra's spatRaster objects, not raster's suite of objects. As such, we will need to uninstall the latest version of blockCV if installed, and install an older
# version. Hopefully, ENMeval will be updated to work with terra and not raster in the near future and this step won't be necessary.
if("blockCV" %in% installed.packages()[,"Package"]){
  remove.packages("blockCV")
}
require(devtools)
install_version("blockCV", version = "2.1.1", repos = "http://cran.us.r-project.org")
# load older version. 
library(blockCV)
...
- Lamigueiro, O. P., & Hijmans, R. (2021). rasterVis. R package version 0.50.2. URL: [https://oscarperpinan.github.io/rastervis/](https://oscarperpinan.github.io/rastervis/).
